...
def export_to_csv(logs, output_file):
    ...
    ) as csvfile:  # utf-8-sig para Excel
...
def show_summary(logs):
    ...
    if hour_counts:
        ...
        print(
            "  - Hora pico: "
            f"{peak_hour[0]}:00-{peak_hour[0]}:59 ({peak_hour[1]} accesos)"
        )
...
def export_user_report(user_email, output_file=None):
    ...
    try:
        ...
        with open(output_file, "w", newline="", encoding="utf-8-sig") as csvfile:
            ...
            csvfile.write("REPORTE DE USUARIO\n")
            csvfile.write("==================\n")
            ...
            csvfile.write(
                f"Registrado: {user.created_at.strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--today", action="store_true", help="Exportar solo registros de hoy"
    )
    parser.add_argument(
        "--week", action="store_true", help="Exportar registros de la última semana"
    )
    parser.add_argument(
        "--month", action="store_true", help="Exportar registros del último mes"
    )
    parser.add_argument(
        "--user", type=str, help="Generar reporte de un usuario específico (email)"
    )
